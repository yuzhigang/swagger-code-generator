import request from '@/utils/request'
import { CraneDto, SaveCraneDto, CraneDtoPagedResultDto } from './definitions'

export default class CraneService {
  static async create(body: SaveCraneDto) {
    const url = '/api/lms/crane'
    return request<CraneDto>(url, {
      method: 'post',
      data: body
    })
  }

  static async getList(query: {
    kind?: 0 | 1
    isActive?: boolean
    sorting?: string
    skipCount?: number
    maxResultCount?: number
  }) {
    const url = '/api/lms/crane'
    return request<CraneDtoPagedResultDto>(url, {
      method: 'get',
      params: query
    })
  }

  static async update(id: string, body: SaveCraneDto) {
    const url = `/api/lms/crane/${id}`
    return request<CraneDto>(url, {
      method: 'put',
      data: body
    })
  }

  static async delete(id: string) {
    const url = `/api/lms/crane/${id}`
    return request(url, {
      method: 'delete'
    })
  }

  static async get(id: string) {
    const url = `/api/lms/crane/${id}`
    return request<CraneDto>(url, {
      method: 'get'
    })
  }
}
