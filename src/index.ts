...
export async function codegen(options: ISwaggerOptions) {
  ...
  setDefinedGenericTypes(options.extendGenericType)
  // 获取swagger.json 定义文件
  if (options.remoteUrl) {
    ...
    swaggerSource = <ISwaggerSource>options.source
  } else {
  ...
  if (options.multipleFileMode) {
    ...
    Object.entries(requestCodegen(swaggerSource.paths, isV3, options)).forEach(([className, requests]) => {
      ...
      requests.forEach(req => {
        ...
        // if (options.isUmiRequest) {
        text += requestTemplateForUmi(reqName, req.requestSchema, options)
        // } else {
      ...
      text = serviceTemplate(className + options.serviceNameSuffix, text, uniqueImports, options.extraImports)
      services.push(className + options.serviceNameSuffix)
      writeFile(options.outputDir || '', className + 'Service.ts', format(text, options))
    })

    const importServices = services.map(s => {
    ...
    writeFile(options.outputDir || '', 'index.ts', format(exportServices, options))
